...
SOURCES = chart.cc layout.cc ace.cc draw.cc point-style.cc map-elements.cc
...
LDLIBS = -L$(LIB_DIR) -lacmacsbase -lacmacsdraw -llocationdb -lhidb -lboost_filesystem -lboost_system $$(pkg-config --libs cairo) $$(pkg-config --libs liblzma) $$($(PYTHON_CONFIG) --ldflags | sed -E 's/-Wl,-stack_size,[0-9]+//')
